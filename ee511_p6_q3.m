%%-------------------------------------------------------------------------
%%Project-6:: Question - 3
%%To Simulate Alpha Stable Distributions
%%Author                Date               Revision
%%Rajasekar Raja     03/27/17         Initial Revision
%%-------------------------------------------------------------------------
function [] = ee511_p6_q3(alpha,beta,gamma,delta,varargin)
  %--------------------------------------------------------------------------
  ...
  % R = STBLRND(ALPHA,BETA,GAMMA,DELTA,[M,N,...]) returns an M-by-N-by array.    
  %--------------------------------------------------------------------------
  if nargin < 4
  ...
  %Generate sample: See if parameters reduce to a special case, if so be quick, if not perform general algorithm
  ...
  figure(1)
  hist(r);
  title('The histogram to plot the number of samples');
  xlabel('The range of X ');
  ylabel('The frequency of the samples lying in the range of X');
  figure(2)
  plot( xlength , stblpdf(xlength,alpha,beta,gamma,delta,'quick'));
  title('The Theo PDF of the alpha stable random variable');
  xlabel('The range of X ');
  ylabel('The probability(PDF) for different values of x');
  figure(3)
  title('The graph shows the Theo pmf overlayed on the histogram of random variable generated by M ');
  yyaxis left
  y=hist(r,xlength);
  bar(xlength,y);
  ...
  figure(4)
  time_plot = timeseries(r,1:1000);
  time_plot.Name = 'Time Series Plot';
  plot(time_plot)
  end
  ...
  %Compute pdf:Check to see if you are in a simple case, if so be quick, if not do general algorithm
  if alpha == 2                  % Gaussian distribution 
  ...
  elseif abs(alpha - 1) > 1e-5          % Gen. Case, alpha ~= 1
          ...
          elseif ~quick % Locate peak in integrand and split up integral        
              ...
              p(x > zeta) = c2 .* ...
                 ...
                 + theta2shift2 .* quadv(@(theta) zexpz( g2(theta) ),...
                                         .5 , 1, tol) );                                                     
          else  % be quick - calculate integral without locating peak. Use a default tolerance of 1e-6
      ...
      % x < zeta, recall function with -xold, -beta, -delta(No rescale needed)    
      if any(x(:) < zeta)
